...
import { 
  ArrowLeft, LayoutDashboard, AlertTriangle, BrainCircuit, FileDown, Camera, Sun, Moon, Landmark, CreditCard, ShoppingBag, Search
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const chartData: ChartData[] = useMemo(() => {
    ...
    return Object.entries(spendingByCategory).map(([name, value]) => ({
      name: language === 'en' && CATEGORY_MAPPING[name] ? CATEGORY_MAPPING[name] : name,
      value,
  ...
  if (!user) {
    return <LoginScreen onLogin={handleLogin} translations={t} language={language} />;
  }
  ...
  return (
    <div className={`min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300 ${language === 'ar' ? 'rtl' : 'ltr'}`} dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <main className="container mx-auto px-4 py-8 pb-24 md:pb-8">
        {activeView === 'home' && <HomeView t={t} onScanClick={() => setIsScanning(true)} user={user} />}
        {activeView === 'dashboard' && (
            <div className="space-y-6 animate-fadeIn">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="h-96">
                        <BudgetStatus budgets={settings.budgets} currentSpending={spendingByCategory} translations={t} language={language} />
                    </div>
                </div>
        ...
        {activeView === 'payments' && (
          <div className="space-y-6 animate-fadeIn">
            ...
            <RecentInvoices invoices={invoices.filter(i => i.paymentMethod === 'CARD')} translations={t} language={language} />
          </div>
        )}
      </main>
      {isScanning && <ScannerModal onClose={() => setIsScanning(false)} onInvoiceAdded={(inv) => setInvoices([inv as any, ...invoices])} defaultCurrency={settings.defaultCurrency} translations={t} accountType={user.accountType} invoices={invoices} />}
